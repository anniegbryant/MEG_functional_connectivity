...
if op.isfile(f"{output_feature_path}/sub-{subject_id}_ses-{visit_id}_all_pyspi_results_individual_epochs_{SPI_subset_base}_{duration}.csv"):
...
all_epoch_files = [f for f in os.listdir(f"{time_series_path}/sub-{subject_id}_ses-{visit_id}_epochs") if "Category_Selective" in f]
...
for csv_file in all_epoch_files:
    ...
    for meta_ROI in region_names:
        try:
            this_ROI_data = pd.read_csv(f"{time_series_path}/sub-{subject_id}_ses-{visit_id}_epochs/{stimulus_type}_{relevance}_{duration}_{epoch_number}_{meta_ROI}.csv", encoding='latin-1')
            ...
            print(f"Failed for {time_series_path}/sub-{subject_id}_ses-{visit_id}_epochs/{stimulus_type}_{relevance}_{duration}_{epoch_number}_{meta_ROI}.csv")
            # Quit the script
            exit()
    ...
    for stimulus_presentation in ['on', 'off']:
        try:
            ...
            print(f"Failed for {stimulus_type}_{relevance}_{duration}_{epoch_number}_{stimulus_presentation}")

all_pyspi_results_for_this_subject = pd.concat(all_pyspi_results_for_this_subject_list, axis=0).reset_index()
all_pyspi_results_for_this_subject.to_csv(f"{output_feature_path}/sub-{subject_id}_ses-{visit_id}_all_pyspi_results_individual_epochs_{SPI_subset_base}_{duration}.csv", index=False)