...
##################################################################################################
# Running catch24 across subjects, averaged epochs
##################################################################################################

# Averaged epochs
cmd="qsub -o $github_dir/cluster_output/Cogitate_MEG_catch24_averaged_^array_index^.out \
   -J 1-94 \
   -N catch24_MEG \
   -l select=1:ncpus=1:mem=20GB:mpiprocs=1 \
   -v input_model_file=$input_model_file,SPI_subset=fast \
   run_catch24_for_subject_averaged_epochs.pbs"
$cmd
