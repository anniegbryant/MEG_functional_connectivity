...
metadata_dir = f"{base_repo}/data/metadata"
...
def run_pyspi_for_df(subject_id, df, calc):
    ...
    # Also take the absolute value of the data
    TS_array_abs = np.abs(TS_array)

    # Load data
    calc_copy_abs.load_dataset(TS_array_abs)
    calc_copy_abs.compute()

    SPI_res_abs = deepcopy(calc_copy_abs.table)

    # Iterate over each SPI
    SPI_res_abs.columns = SPI_res_abs.columns.to_flat_index()

    SPI_res_abs = SPI_res_abs.rename(columns='__'.join).assign(meta_ROI_from = lambda x: x.index)
    SPI_res_long_abs = SPI_res_abs.melt(id_vars='meta_ROI_from', var_name='SPI__meta_ROI_to', value_name='value')

    SPI_res_long_abs["SPI"] = SPI_res_long_abs["SPI__meta_ROI_to"].str.split("__").str[0]
    SPI_res_long_abs["meta_ROI_to"] = SPI_res_long_abs["SPI__meta_ROI_to"].str.split("__").str[1]

    SPI_res_long_abs = (SPI_res_long_abs
                        .drop(columns='SPI__meta_ROI_to')
                        .query('meta_ROI_from != meta_ROI_to')
                        .assign(meta_ROI_from = lambda x: x['meta_ROI_from'].map(ROI_lookup),
                                meta_ROI_to = lambda x: x['meta_ROI_to'].map(ROI_lookup))
    ...
    return pd.concat([SPI_res_long, SPI_res_long_abs])

    
def process_for_subject(subject_id, visit_id, duration, time_series_path, output_feature_path, SPI_subset, SPI_subset_base):
...
subject_list = np.loadtxt(f"{metadata_dir}/subject_list_Cogitate_MEG_with_all_data.txt", dtype=str)  
